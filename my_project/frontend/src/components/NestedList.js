...
function NestedList() {
  ...
  useEffect(() => {
    axios.get('https://phoenician.site/api/items/')
  ...
  const toggleExpand = async (itemCode) => {
    ...
    if (!expandedItems[itemCode]) {
      try {
        const response = await axios.get(`https://phoenician.site/api/items/?item_code=${itemCode}`);
...
// function NestedList({ items, expandedItems, onExpand }) {
...
//     return items
//       .filter(item => {
//         const itemLevel = item.ITEM ? item.ITEM.split('.').length : 1;
//         return item.ITEM && item.ITEM.startsWith(parentCode ? `${parentCode}.` : '') && itemLevel === level;
//       })
//       .map((item) => {
//         const itemCode = item.ITEM;
//         const hasChildren = items.some(child => child.ITEM.startsWith(`${itemCode}.`) && child.ITEM.split('.').length === level + 1);
...
//         return (
//           <div key={itemCode || Math.random()} id={itemCode} className="nested-item" style={{ marginLeft: `${level * 20}px` }}>
//             {/* Display item header with an arrow if it has children */}
//             {itemCode ? (
//               <div className="nested-item-header" onClick={() => hasChildren && onExpand(itemCode)}>
//                 {hasChildren && (
//                   <span className="expand-arrow">
//                     {expandedItems[itemCode] ? '▼' : '▶'}
//                   </span>
...
//             {/* Display the table directly if no ITEM code (leaf node) or if expanded */}
//             {(expandedItems[itemCode] || !hasChildren || !itemCode) && (
//               <table className="nested-item-table">
//                 <thead>
//                   <tr>
//                     <th>Description</th>
