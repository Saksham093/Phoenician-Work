...
function App() {
    ...
    useEffect(() => {
    //   axios.get('https://api.phoenician.site/api/projects', {
      axios.get('https://api.phoenician.site/', {
          headers: {
    ...
    const expandItem = async (itemCode) => {
        ...
        if (!items.some(item => item.ITEM && item.ITEM.startsWith(`${itemCode}.`))) {
            try {
                const response = await axios.get(`https://api.phoenician.site/${itemCode}/details`);
